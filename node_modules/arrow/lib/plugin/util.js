...
function printSpecifyNodeVersionWarningIfRequired(productionNodeVersion, logger) {
	// For 2.0, this is a no-op.  The cloud will default to node 1.6.0, and on-prem,
	// there is only one version available 1.6.0.  However, the cloud can support
	// additional versions, so the following code is retained.  It would be better
	// to add a way to detect cloud vs. on-prem and then warn if cloud.  That will
	// need to be done after 2.0.
	return;
