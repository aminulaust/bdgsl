    ...
    temp.mkdir('acs',function(err, tmpdir) {
        ...
        for ( var c = 0; c < dirs.length; c++) {
            ...
            if (c > 0) {
                ...
                if (bn === 'node_modules') {
                    ...
                    for ( var x = 0; x < listdir.length; x++) {
                        ...
                        if (!u.exists(ltd)) {
                            fsExtra.mkdirpSync(ltd);
                        }
                        fsExtra.copySync(path.join(dir, listdir[x]), ltd);
                    }
                    continue;
                } else {
            ...
            removeDependencies(target, function(err, packagesBack){
                ...
                // Remove logs folder
                fsExtra.removeSync(path.join(target, 'logs'));
...
function removeDependencies(dir, callback) {
    var npm = u.getNpm();
    npm.load({prefix : dir, loglevel : 'error', progress: false}, function(err) {
        if(err){
