...
function stringifyJSON(j) {
    var final_code = UglifyJS.minify('(' + JSON.stringify(j) + ')', {
        fromString: true,
        compress: false,
        output: {
            beautify: true,
    ...
    return final_code.substring(1, final_code.length - 2); // remove () needed for parsing
}
...
function getNpm() {
    try {
        ...
        if(E.toString().indexOf('Cannot find module') !== -1) {
            die("npm not found. Please install npm and try again." + E);
        } else {
            die(E.toString());
        }
    }
