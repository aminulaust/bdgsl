...
var host = u.getGlobalConfig().publishHost || u.getDefaultConfig().publishHost;
    ...
    this.handler = function(error, response, body) {
        try {
            ...
            if (response.statusCode === 404) {
                var nosup = 'This operation is not supported in ArrowCloud';
                try {
                    if(body) {
                        var jsonBody = JSON.parse(body);
                        if(jsonBody && jsonBody.clusterVersion) {
                            nosup += ' ' + jsonBody.clusterVersion;
                        }
                    }
                } catch(E){};
                nosup += ' (' + host + ')!';
            ...
            if( result.errcode && result.errcode === 100 ) {
                ...
                u.requireLogin(function() {
                    ...
                    if( !_.contains(['publish', 'config', 'crt', 'new', 'cname', 'domain', 'server', 'restart', 'push'], params[0]) ){
                        process.stdin.destroy();
                    }
