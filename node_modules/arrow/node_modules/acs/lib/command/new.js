...
exports.run = function(args, program) {
    ...
    if (u.exists(projectdir)) {
        if( !program.force) {
            ...
            var dockerfile = path.join(projectdir, 'Dockerfile');
    ...
    var type = 'free';
    if (program.type) {
        if(program.type === 'free' || program.type === 'image') {
            type = program.type;
        } else {
            u.die('Invalid value \'' + program.type + '\' for --type option. Availabe value is \'free\' and \'image\'.');
        }
    }
        ...
        if (!result.success) {
            if(result.errcode === 265) {
                ...
                program.choose(orgs, function(i) {
                    var orgid = orgids[i];
                    console.log('Creating new Arrow Cloud app for organization ' + orgs[i] + '...');
                    request({
                        uri: encodeURI(url),
                        method: 'POST',
                        proxy: proxy,
    ...
    var succeed = function(response, result) {
        if (!u.exists(projectdir)) {
            ...
            case 'image':
                // generate the default service index
                u.generateTemplate('image/app.js', {}, path.join(projectdir, 'app.js'));

                // generate the default README.md
                u.generateTemplate('image/README.md', {}, path.join(projectdir, 'README.md'));

                // generate the new config file
                serviceDef = { "name" : name };
                addServiceInfo(serviceDef);

                u.generateTemplate('image/package.json', serviceDef, path.join(projectdir, 'package.json'), true);

                // generate the default Dockerfile
                u.generateTemplate('image/Dockerfile', {}, path.join(projectdir, 'Dockerfile'));

                // generate the default start_app script
                u.generateTemplate('image/start_app', {}, path.join(projectdir, 'start_app'));
                break;                 
            }
