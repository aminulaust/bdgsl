...
describe('#content', function () {
	...
	it('should generate full code block', function () {
		...
		var result = content.generate(md);
		var expected = fs.readFileSync('test/results/fullCodeBlock.html', 'UTF-8');
		var actual = result.markup.replace(/\s+/g, ' ').trim();
	...
	it('should not hang with certain underscore permutations', function () {
		...
		var result = content.generate(md);
		var expected = fs.readFileSync('test/results/underscorePermutations.html', 'UTF-8');
		var actual = result.markup.replace(/\s+/g, ' ').trim();
		expected = expected.replace(/\s+/g, ' ').trim();
		result.should.be.an.object;
		actual.should.be.equal(expected);
	...
	it('should generate code and table', function () {
		...
		var expected = fs.readFileSync('test/results/generateCodeAndTable.html', 'UTF-8');
		var actual = result.markup.replace(/\s+/g, ' ').trim();
		expected = expected.replace(/\s+/g, ' ').trim();
		result.should.be.an.object;
		actual.should.be.equal(expected);
