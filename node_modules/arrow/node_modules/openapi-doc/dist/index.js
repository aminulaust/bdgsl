...
var Swagger = function () {
  ...
  */
   ...
   * @public
   * @param {object} repsonses - A valid Swagger responses object.
   */
		...
		value: function responses(_responses) {
			...
			Object.keys(_responses).forEach(function (code) {
				...
				var typeref = void 0;
				var type = void 0;
				if (_responses[code].schema) {
					if (_responses[code].schema.type === 'array') {
						isArray = true;
						typeref = _responses[code].schema.items['$ref'];
					} else {
						typeref = _responses[code].schema['$ref'];
					}
					if (typeref && typeref.startsWith('#/definitions/')) {
						type = typeref.substr(14);
					}
				}
				_this5.response(code, _responses[code].description || null, type, isArray);
			});
			return this;
		}
   ...
   * @param {string} [prefix] - All paths will be prefixed with `prefix`.
   */

	}, {
		key: 'merge',
		value: function merge(swagger, prefix) {
			...
			return this.paths(swagger.paths, prefix);
		}
   ...
   *
   * @public
   * @param {object} paths - A well defined swagger paths object.
   * @param {string} [prefix] - All paths will be prefixed with `prefix`.
   */

	}, {
		key: 'paths',
		value: function paths(_paths, prefix) {
			var _this6 = this;

			Object.keys(_paths).forEach(function (part) {
				var path = part;
				if (prefix) {
					path = urljoin(prefix, part);
				}
				Object.keys(_paths[part]).forEach(function (verb) {
					var orig = _paths[part][verb];

					// create the swagger path
					if (!_this6[verb]) {
						// throw new Error(`illegal verb: ${verb}`);
					...
					// clone properties
					['summary', 'description', 'operationId', 'consumes', 'produces', 'parameters', 'responses', 'tags'].forEach(function (key) {
						if (orig.hasOwnProperty(key) && orig[key] !== '') {
							// clone the component and set in this swagger
							_this6[key](JSON.parse(JSON.stringify(orig[key])));
						}
					});
				});
			});
